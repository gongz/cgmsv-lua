...
local function processGroup(groupPoints)
    ...
    for _, cluster in ipairs(clusters) do
        ...
        if diffX > minDiff then dirH = "东"
        elseif diffX < -minDiff then dirH = "西" end

        if diffY > minDiff then dirV = "南"
        elseif diffY < -minDiff then dirV = "北" end
...
function getWarpModule:get_warp(fd,head,data)
    ...
    local maptype = Char.GetData(player, CONST.对象_地图类型)
    local mapid = Char.GetData(player, CONST.对象_地图)
                ...
                for _, obj in ipairs(objtbl) do
                    ...
                    elseif objtype == 1 then
                        ...
                        if charIndex and charIndex >= 0 then
                            local npcEventType = Char.GetData(charIndex, CONST.对象_NPC_EVENT_TYPE)
        ...
        for l = 1,maxy do
            ...
            if j > 0 then
                for key, v in ipairs(_objtbl) do
                    ...
                    local _Name = Char.GetData(_Index,CONST.对象_名字)
